...
If a Trellis task is active and `implement.jsonl` / `check.jsonl` still contain
only `_example`, run the local recommendation helper before implementation:

```bash
python3 ./.trellis/scripts/recommend_context.py <task-dir> --write
```

This scans the task plan, PRD, research notes, and path keywords to append the
relevant `.trellis/spec/` and task research files.
