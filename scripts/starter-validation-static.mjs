...
export function normalizeQualityMode(quality = 'standard') {
...
function normalizeDefaultLocale(locale = 'zh-CN') {
  if (locale !== 'zh-CN' && locale !== 'en-US') {
    throw new Error(`Unknown starter locale "${locale}". Supported locales: zh-CN, en-US`)
  }
...
function validateDefaultSwitcherOutput(files, textEntries, themes, i18nEnabled, defaultLocale) {
  const failures = []
  const localeCatalogs = ['zh-CN', 'en-US']
  const expectedLocales = i18nEnabled ? localeCatalogs : [defaultLocale]
  const actualLocales = localeCatalogs.filter((locale) => files.includes(`src/i18n/locales/${locale}.ts`))

  if (!arraysEqual(actualLocales, expectedLocales)) {
    failures.push(
      createFailure(
        'locale-catalogs-match-selection',
        `Generated locale catalogs must match the selected locale capability. Expected ${expectedLocales.join(', ')}; found ${actualLocales.join(', ') || 'none'}.`,
        'src/i18n/locales/'
      )
    )
  }

  const i18nIndex = getTextEntry(textEntries, 'src/i18n/index.ts')?.text ?? ''
  if (!i18nIndex.includes(`export const DEFAULT_LOCALE = '${defaultLocale}'`)) {
    failures.push(createFailure('locale-default-match-selection', `Generated DEFAULT_LOCALE must be "${defaultLocale}".`, 'src/i18n/index.ts'))
  }

  const indexHtml = getTextEntry(textEntries, 'index.html')?.text ?? ''
  if (!indexHtml.includes(`<html lang="${defaultLocale}">`)) {
    failures.push(createFailure('locale-html-lang-match-selection', `Generated index.html must declare lang="${defaultLocale}".`, 'index.html'))
  }
...
export async function validateGeneratedStarterStatic(projectDir, options = {}) {
  ...
  const defaultLocale = normalizeDefaultLocale(options.locale)
  ...
  failures.push(...validateDefaultSwitcherOutput(files, textEntries, themes, i18nEnabled, defaultLocale))
