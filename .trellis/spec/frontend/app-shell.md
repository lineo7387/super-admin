...
Stacked-group decoration must sit behind each thumbnail's own preview surface at the card level. Do not stack additional window previews inside `StageWindowPreview`, and do not let a group stack background replace or tint the individual window/card preview background. The visible preview remains the group's current active window; the folded windows are represented by non-interactive card backs offset behind it, currently biased to the lower-left so the stack reads as a macOS-like window pile.
...
- Stage Manager may use layout/theme tokens for visual language, but the open route model, keep-alive cache, shared window metadata, and pin/refresh/close behavior remain owned by the workspace tab store plus shared Stage Manager composables.
...
- Stage Manager uses the same tab store. Fullscreen Overview and side Stage Rail must reflect pin/refresh/close behavior from Workspace Tabs through shared window metadata/action logic. The side Stage Rail remains activation-first, so those controls are reveal-on-interaction rather than persistent chrome.
...
- Browser check fullscreen Overview and side Stage Rail show the same pinned/refresh/close controls on hover, touch/active, or focus; Stage Rail remains window-only and does not add permanent action chrome.
