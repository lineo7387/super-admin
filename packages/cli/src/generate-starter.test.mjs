...
describe('create-super-admin starter generation', () => {
  ...
  it('generates the default single-theme starter and passes static validation', async () => {
    ...
    expect(agentsMd).toContain('唯一 AI 开发入口')
    expect(agentsMd).toContain('@ai-context/core.md')
    expect(agentsMd).toContain('@ai-context/data-flow.md')
    expect(agentsMd).toContain('@ai-context/extension-points.md')
    expect(agentsMd).not.toContain('theme.md')
    expect(agentsMd).not.toContain('i18n.md')
    expect(agentsMd).not.toContain('charts.md')
    expect(coreContext).toContain('这是用户项目，不是 Super Admin 源码仓库')
  ...
  it('generates the optional ECharts template when selected', async () => {
    ...
    expect(agentsMd).toContain('@ai-context/charts.md')
    expect(chartsContext).toContain('当前项目生成了 ECharts 图表示例能力')
    expect(chartsContext).toContain('dependencies: `echarts`, `vue-echarts`')
  ...
  it('generates selected multi-theme and i18n variants', async () => {
    ...
    const packageJson = await readGeneratedJson(input.targetDirectory, 'package.json')
    const agentsMd = await readGeneratedText(input.targetDirectory, 'AGENTS.md')
    const themeContext = await readGeneratedText(input.targetDirectory, 'ai-context/theme.md')
    const i18nContext = await readGeneratedText(input.targetDirectory, 'ai-context/i18n.md')
    ...
    expect(agentsMd).toContain('@ai-context/theme.md')
    expect(agentsMd).toContain('@ai-context/i18n.md')
    expect(themeContext).toContain('已安装 themes: `base`, `cyberpunk`')
    expect(themeContext).toContain('默认 theme: `base`')
