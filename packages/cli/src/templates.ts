...
- App module registry：通用注册校验位于 \`src/modules/app-module-registry.ts\`，app-local composition 位于 \`src/modules/module-registry.ts\`。可选模块把默认入口和兼容 redirects 放在自身 registration 中；Examples 使用 \`src/modules/examples/examples.registration.ts\`，因此删除 Examples 时不需要修改 router、auth 或 workspace fallback。
...
3. 如果生成了 ECharts，同时删除 \`src/modules/charts/\`、\`src/shared/charts/\`、\`echarts\` / \`vue-echarts\` 依赖和 \`ai-context/charts.md\`。
4. 运行 \`${input.packageManager} run check\`；minimal 项目改为运行 typecheck 与 build。
