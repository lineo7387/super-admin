...
<template>
  <div class="flex flex-wrap items-center justify-between gap-3 border-t border-[var(--border)] px-4 py-3 text-sm text-[var(--muted-foreground)]">
    ...
    <div class="flex items-center gap-2">
      <AdminButton size="sm" variant="secondary" :disabled="range.page <= 1" @click="emit('pageChange', range.page - 1)">{{
        props.previousLabel ?? 'Previous'
      }}</AdminButton>
      <span class="min-w-16 text-center">{{ props.pageLabel ?? `Page ${range.page} / ${range.pageCount}` }}</span>
      <AdminButton size="sm" variant="secondary" :disabled="range.page >= range.pageCount" @click="emit('pageChange', range.page + 1)">{{
