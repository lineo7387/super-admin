...
4. 如果项目生成了 ECharts，继续删除 `src/modules/charts/`、`src/shared/charts/`、`echarts` / `vue-echarts` dependencies 和 `ai-context/charts.md`，从 `AGENTS.md` 移除对应 import，并把 `super-admin.config.ts` 的 chart provider 改回 `none`。
5. Standard 项目运行 `pnpm check`；minimal 项目运行 `pnpm typecheck && pnpm build`。
