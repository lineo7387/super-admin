...
如果 selected package 的当前版本已经存在于 npm，workflow 会在 install、build 和 publish 前失败。此时应先补 changeset、运行 `pnpm release version` 生成新的不可变版本，再重新触发 workflow；不要尝试覆盖已发布版本。

Registry timeout、非 404 错误或异常 metadata 也会让预检直接失败。只有明确的 404 才表示该 package 尚未发布，workflow 不会在无法确认版本状态时继续 publish。

版本预检通过后，workflow 会运行 `pnpm release check`。Package `prepublishOnly` guards 会阻止意外的 normal local publishes，并且只允许来自配置好的 GitHub Actions workflow、带 `next` tag 和 provenance 的 normal publishes。Workflow 只发布 dependency-aware selected package set。
